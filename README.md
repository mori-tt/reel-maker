...
- シネマティック（レターボックス）／ダイナミック（フラッシュカット）／ミニマル（ビネット）／フォトアルバム（暖色フレーム）／SNSトレンド（アクセントバッジ）の5種類の動画演出。動きの速さだけでなく、色調・装飾も演出ごとに変えて見た目を差別化
...
- フェード＋ズーム付き9:16プレビュー
- 1080×1920 MP4（H.264）動画の生成・ダウンロード。WebCodecs非対応ブラウザではWebMにフォールバック
...
書き出しは [mediabunny](https://mediabunny.dev/) 経由のWebCodecs（`VideoEncoder`）を優先して使います。画像1枚ごとにcanvasへ描画し、実時間とは無関係に明示的なタイムスタンプでエンコードするため、端末が重くてもフレームが欠落・重複しません（`canvas.captureStream` + `MediaRecorder`によるリアルタイム録画は、描画が1フレーム分の時間に間に合わないとその場でコマ落ちします）。副次的に、コンテナも汎用性の高いMP4（H.264）になり、Safari 16.4以降でも書き出せます。

WebCodecsが使えないブラウザ（古いFirefoxなど）では、従来どおり`canvas.captureStream` + `MediaRecorder`によるWebM書き出しに自動フォールバックします。

## AIコピー提案

先に「画像ごとのコピー一覧」で文字を入れたい画像の「文字を表示」にチェックを入れてから生成してください。AIはチェックした画像だけにタイトル・CTAを書き込み、チェックしていない画像には触れません（画像を追加した直後はどれもチェックなしの状態です）。何も選択せずに生成しようとするとボタンが無効化され、案内が表示されます。
