...
export async function getChromeAiAvailability(api = getChromeLanguageModel(), language: Language = 'ja'): Promise<ChromeAiAvailability> { if (!api) return 'unavailable'; try { return await api.availability(modalities(language)) } catch { return 'unavailable' } }
...
export async function prepareChromeAi(options: { onDownloadProgress?: (progress: number) => void; api?: LanguageModelApi | null; language?: Language } = {}): Promise<ChromeAiAvailability> {
  const language = options.language ?? 'ja'; const api = options.api ?? getChromeLanguageModel(); if (!api) return 'unavailable'
  const availability = await getChromeAiAvailability(api, language); if (availability === 'unavailable' || availability === 'downloading') return availability
...
export async function generateChromeAiCopy(options: { image: Blob; direction?: string; customDirection?: string; formatName?: string; language?: Language; onDownloadProgress?: (progress: number) => void; api?: LanguageModelApi | null }): Promise<AiCopy> {
  const language = options.language ?? 'ja'; const api = options.api ?? getChromeLanguageModel(); if (!api) throw new Error(messages[language].missing)
  const availability = await getChromeAiAvailability(api, language); if (availability === 'unavailable' || availability === 'downloading') throw new Error(chromeAiAvailabilityMessage(availability, language))
