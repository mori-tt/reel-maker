...
import { PLATFORMS, VIDEO_FORMATS, VIDEO_PATTERNS, VIDEO_QUALITIES, TRANSITION_TYPES, GOOGLE_FONTS, FONT_FAMILY_OPTIONS, DEFAULT_COLOR_ADJUSTMENTS, DEFAULT_BRAND_COLORS, buildColorFilter, loadGoogleFonts, getFrameState, getFrameStateWithDurations, totalDurationWithDurations, getPatternFrame, getVideoFormat, getVideoPattern, getVideoQuality, isVideoFormatId, isVideoPatternId, isVideoQualityId, maxSecondsPerImage, minSecondsPerImage, moveItem, watermarkRect, PURPOSES, STILL_IMAGE_PURPOSES, MEDIA_TYPES } from './reel'
import type { PatternFrame, Platform, PurposeId, MediaType, VideoFormatId, VideoPattern, VideoPatternId, VideoQualityId, WatermarkPosition, TransitionType, ColorAdjustments, BrandColors, SubtitleConfig } from './reel'
...
import { loadVideoExport } from './video-export-loader'
...
const [mediaType, setMediaType] = useState<MediaType>(() => { const stored = localStorage.getItem('videoMediaType'); return stored === 'stillImage' ? 'stillImage' : 'video' })
  ...
  const prepareBgm = async (durationSeconds: number, sampleRate: number, numberOfChannels: number) => { if (!bgmFile) return null; const { pickAudioCodec } = await loadVideoExport(); const codec = await pickAudioCodec(numberOfChannels, sampleRate); if (!codec) return null; const context = new AudioContext(); try { const decoded = await decodeAudioFile(bgmFile, context); return { buffer: fitAudioBuffer(context, decoded, durationSeconds, bgmVolume), codec } } finally { context.close() } }
...
<div className="media-type-row">{MEDIA_TYPES.map(item=><button key={item.id} type="button" className={`media-type-tab ${mediaType===item.id?'active':''}`} aria-pressed={mediaType===item.id} onClick={()=>{setMediaType(item.id);localStorage.setItem('videoMediaType',item.id)}}><span>{item.name[language]}</span></button>)}</div>
{PLATFORMS.map(platform=><div className="format-platform-group" key={platform}><span className="format-platform-label">{t(platformLabelKey(platform))}</span><div className="format-grid">{VIDEO_FORMATS.filter(item=>item.platform===platform).map(item=><button key={item.id} className={formatId===item.id?'active':''} aria-pressed={formatId===item.id} onClick={()=>{setFormatId(item.id);localStorage.setItem('videoFormat',item.id)}}><b>{item.name[language]}</b><small>{item.description[language]}</small></button>)}</div></div>)}
{mediaType === 'video' ? (
