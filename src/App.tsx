...
import { PLATFORMS, VIDEO_FORMATS, VIDEO_PATTERNS, VIDEO_QUALITIES, TRANSITION_TYPES, GOOGLE_FONTS, FONT_FAMILY_OPTIONS, DEFAULT_COLOR_ADJUSTMENTS, DEFAULT_BRAND_COLORS, buildColorFilter, loadGoogleFonts, getFrameState, getFrameStateWithDurations, totalDurationWithDurations, getPatternFrame, getVideoFormat, getVideoPattern, getVideoQuality, isVideoFormatId, isVideoPatternId, isVideoQualityId, maxSecondsPerImage, minSecondsPerImage, moveItem, watermarkRect, PURPOSES } from './reel'
...
const [purposeId, setPurposeId] = useState<PurposeId>(() => { const stored = localStorage.getItem('videoPurpose'); return stored ? (PURPOSES.find(p => p.id === stored)?.id ?? 'standard') : 'standard' })
  ...
  return <div className="app-shell"><header><div className="brand"><span className="brand-mark">F</span><span>Frameflow</span></div><div className="header-actions"><span className="eyebrow">MULTI-FORMAT VIDEO STUDIO</span><div className="language-switch" role="group" aria-label={t('language')}><span>{t('language')}</span><button className={language==='en'?'active':''} aria-pressed={language==='en'} onClick={()=>changeLanguage('en')}>EN</button><button className={language==='ja'?'active':''} aria-pressed={language==='ja'} onClick={()=>changeLanguage('ja')}>日本語</button></div></div></header><main><section className="intro"><p className="kicker">IMAGES TO MOTION</p><h1>{language==='en'?<>Turn photos into <em>moving stories.</em></>:<>写真から、<em>動く物語</em>を。</>}</h1><p>{language==='en'?'Create polished Reels, Stories, posts and Shorts with format-aware high-quality settings.':'リール、ストーリー、フィード、Shortsを用途別の高品質設定で仕上げます。'}</p></section><div className="workspace"><section className="editor panel">
    ...
    <section className="format-section"><div className="pattern-head"><div><span>▣</span><div><h3>{t('formatTitle')}</h3><p>{t('optimizeSafeArea')}</p></div></div><strong>{format.width} × {format.height}</strong></div>{PLATFORMS.map(platform=><div className="format-platform-group" key={platform}><span className="format-platform-label">{t(platformLabelKey(platform))}</span><div className="format-grid">{VIDEO_FORMATS.filter(item=>item.platform===platform).map(item=><button key={item.id} className={formatId===item.id?'active':''} aria-pressed={formatId===item.id} onClick={()=>{setFormatId(item.id);localStorage.setItem('videoFormat',item.id)}}><b>{item.name[language]}</b><small>{item.description[language]}</small></button>)}</div></div>)}<div className="quality-row">{VIDEO_QUALITIES.map(item=><button key={item.id} className={qualityId===item.id?'active':''} aria-pressed={qualityId===item.id} onClick={()=>{setQualityId(item.id);localStorage.setItem('videoQuality',item.id)}}><b>{item.name[language]}</b><small>{item.description[language]}</small></button>)}</div>
<div className="purpose-row">{PURPOSES.map(item=><button key={item.id} className={purposeId===item.id?'active':''} aria-pressed={purposeId===item.id} onClick={()=>{setPurposeId(item.id);localStorage.setItem('videoPurpose',item.id)}}><b>{item.name[language]}</b><small>{item.description[language]}</small></button>)}</div>{total>format.recommendedMaxSeconds&&<small className="field-help format-duration-hint">{t('durationOverHint').replace('{seconds}', String(Math.round(total))).replace('{max}', String(format.recommendedMaxSeconds))}</small>}</section>
